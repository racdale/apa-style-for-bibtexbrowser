...
@define('ORDER_FUNCTION_FINE','compare_bib_by_authors');
...
@define('COMMA_NAMES',true);// do have authors in a comma separated form?
...
//     echo $this->getKey();
  ...
  */
  function getLink($bibfield,$iconurl=NULL,$altlabel=NULL,$type=NULL) {
    $show = true;
    if ($altlabel==NULL) { $altlabel=$bibfield; }
    $str = $this->getIconOrTxt($altlabel,$iconurl);
    if ($this->hasField($bibfield)) {
       return '<a'.(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'').' class=linkurl href="http://cognaction.org/rdmaterials/php.cv/pdfs/'.$type.'/'.$this->getField($bibfield).'">'.$str.'</a>';
    }
    return '';
  }
  ...
  function getUrlLink($iconurl = NULL, $label = 'pdf') {
    if ($this->hasField('local-url')) {
      $type = $this->getType();
      $localUrl = $this->getLink('local-url', $iconurl, $label, $type);
      return $localUrl;
    }
  ...
  */
  ...
  function getFormattedAuthors() {
    ...
    if (count($authors)>1) $authors[count($authors)-1] = "&amp; ".$authors[count($authors)-1];
    return $authors;
  }
  ...
  */
  ...
  function getFormattedEditors() {
    ...
    foreach ($this->getEditors() as $editor) {
      $editors[]=$this->addHomepageLink($this->formatEditor($editor));
    }
    $sep = ', ';
    if (count($editors)>1) $editors[count($editors)-1] = "&amp; ".$editors[count($editors)-1];
...
function bib2links_default(&$bibentry) {
  $href = 'href="http://cognaction.org/rdmaterials/php.cv/'.$bibentry->getURL().'"'; /* RICK */
  ...
  if (BIBTEXBROWSER_BIBTEX_LINKS) {
    ...
    $str .= "<a".(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'')." class=\"biburl\" title=\"".$bibentry->getKey()."\" {$href}>bibtex</a>"; /* RICK */
  }
  ...
  if (BIBTEXBROWSER_DOI_LINKS && $bibentry->hasField('doi')) {
    $str .= ' <a href="http://dx.doi.org/'.$bibentry->getField("doi").'" class=doiurl>doi</a>'; /* RICK */
  }
...
function compare_bib_by_authors($a, $b)
{
  return strcmp($a->getFormattedAuthorsImproved(),$b->getFormattedAuthorsImproved());
}
